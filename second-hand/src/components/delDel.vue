<template>
    <div>
        ...
        <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item>以物换物</el-breadcrumb-item>
            <el-breadcrumb-item>以物换物商品</el-breadcrumb-item>
        </el-breadcrumb>
        <el-card>
            <!-- 顶部搜索部分 -->
            <el-row :gutter="20">
            ...
            <!-- 中间数据部分 -->
            <el-row :gutter="20">
                <el-col :span="5" v-for="item in changeList" :key="item.id" :offset="1">
...
export default {
    ...
    mounted() {
        if (window.sessionStorage.getItem('changePageSize')) {
            this.queryInfo.pageSize = window.sessionStorage.getItem('changePageSize');

        }
        if (window.sessionStorage.getItem('changePageNum')) {
            this.queryInfo.pageNum = window.sessionStorage.getItem('changePageNum');
        }
        this.getGoodsList();
    },
    methods: {
        ...
        handleSizeChange(newsize) {
            ...
            window.sessionStorage.setItem('changePageSize', newsize,);
            this.getGoodsList();
        },
        //分页改变事件
        handleCurrentChange(newpage) {
            this.queryInfo.pageNum = newpage;
            window.sessionStorage.setItem('changePageNum', newpage,);
            this.getGoodsList();
        },
        checkDetail(id1) {
            this.$router.push({ path: '/changegoodsdetail', query: { id: id1 } })
        },
        getGoodsList() {

            this.$http.get('changegoods', {
                params: {
                ...
                if (res.data.code == 1) {
                    this.changeList = res.data.data.records;
